...
//! ```
//!
//! Keeping templates in the project (not in the tool) means a team's file
//! conventions are versioned alongside the workflows that consume them, and
//! the tool stays free of any one platform's naming rules.
//!
//! NOTE: this file is currently duplicated in ais-runner (`src/services/
//! msg_template.rs`). Both apps read the same `.ais-runner/message-templates`
//! folder so a project's templates work locally *and* against real Azure. The
//! two copies must stay in sync; extracting to a shared git crate (as already
//! done for `ais-chain`) is the obvious follow-up once the format settles.
