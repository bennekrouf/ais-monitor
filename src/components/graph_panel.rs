...
pub fn GraphPanel(props: GraphPanelProps) -> Element {
    ...
    let graph_data = use_memo({
        let chains = props.chains.clone();
        move || build_graph_data(&chains)
    });
        ...
        Some(data) => {
            ...
            rsx! {
                div {
                    ...
                    // ── Top bar ──────────────────────────────────────────
                    div {
                        style: "display:flex; align-items:center; gap:8px; padding:10px 16px 8px 16px; z-index:6; position:relative; flex-shrink:0; flex-wrap:wrap;",
...
fn build_graph_data(chains: &[ChainDetail]) -> Option<GraphData> {
    ...
    // Build edges from consecutive steps within each chain
